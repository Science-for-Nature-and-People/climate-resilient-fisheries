...
g1 <- ggplot(data_ordered, aes(y=attribute, x=case_study, color=score, size=importance)) +
  ...
  labs(x="", y="", tag="A") +
  # Legend
  scale_color_manual(name="Strength", values=RColorBrewer::brewer.pal(4, "Blues")) +
  # Theme
  theme_bw() + my_theme +
...
# Export plot
ggsave(g, filename=file.path(plotdir, "Fig4_attribute_scores_raster_and_prop_v2.png"), 
