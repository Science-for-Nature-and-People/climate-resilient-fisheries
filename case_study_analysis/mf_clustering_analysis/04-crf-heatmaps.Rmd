...
```{r}
# Prep the data to separate out the appropriate cases for score grouping
...
KT_AH_KM_CF_hc <- KT_AH_KM_CF_clust %>% 
  column_to_rownames(var = "case_study") %>% 
  mutate(across(everything(),~scale(.x))) %>% 
  mutate(across(everything(), ~replace_na(.x, 0))) %>% # REVIEW
  as.matrix() %>% 
...
## Grouped Heatmaps by Importance
