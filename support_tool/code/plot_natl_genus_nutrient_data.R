...
plot_natl_genus_nutrition_data <- function(genus_pdiet, genus_pnutrient, cntry, my_theme){
  
  # Seafood consumption time series
  g1 <- ggplot(genus_pdiet %>% filter(country_use==cntry),
               aes(x=year, y=seafood_g_person_day)) +
    geom_line() +
    labs(x="", y="Daily per capita marine seafood supply\n(grams / person / day)", 
         title=paste("GENuS nutrition from seafood:", cntry)) +
    theme_bw() + my_theme
  #g1  
