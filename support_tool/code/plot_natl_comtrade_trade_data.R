...
plot_natl_comtrade_trade_data <- function(data, world_simple, world_centroids, cntry, my_theme){
  ...
  world_data1 <- world_simple %>% 
    ...
    mutate(flow_dir=paste0("Exports (from ", cntry, ")"))
  # Format for plotting
  world_data2 <- world_simple %>% 
    left_join(sdata2, by=c("iso3_use"="iso3")) %>% 
    mutate(flow_dir=paste0("Imports (to ", cntry, ")"))
  ...
  g <- ggplot(world_data, aes(fill=quantity_mt/1000)) +
    ...
    geom_sf(data=world_centroids %>% filter(country_use==cntry), 
            mapping=aes(fill=NULL), color="black", size=3) +
